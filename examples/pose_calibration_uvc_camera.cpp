...
//		else if(strcmp(argv[i], "-output_buffers") == 0) n_output_buffers = atoi(argv[++i]);
    ...
    cv::Point circle_centers[nCircles];
    int current_circle = 0;
    for (int i = 0; i < nCircles; ++i) {
        circle_centers[i] = cv::Point(width / 2, height / 2);
    }
    bool coarse = true;
	...
	while (run) {
		...
		if (uvc_frame_index >= 0) {
			...
			for (int i = 0; i < nCircles; ++i) {
				circle(frame, circle_centers[i], 5, Scalar(0x00, 0xff, 0x00), 2, 8, 0);
			}
//			circle(frame, circle_centers[0], 5, Scalar(0xff, 0x00, 0x00), 2, 8, 0);
//			circle(frame, circle_centers[current_circle], 5, Scalar(0x00, 0xff, 0x00), 2, 8, 0);
            ...
            if (ch != -1) {
                switch (ch) {
                    ...
                    case 'F':
                        ...
                        break;
                    case 'w':
                        circle_centers[current_circle].x -= coarse ? 10 : 1;
                        break;
                    case 'W':
                        circle_centers[current_circle].x += coarse ? 10 : 1;
                        break;
                    case 'h':
                        circle_centers[current_circle].y -= coarse ? 10 : 1;
                        break;
                    case 'H':
                        circle_centers[current_circle].y += coarse ? 10 : 1;
                        break;
                    case '+':
                        circle_centers[current_circle].x += 10;
                        circle_centers[current_circle].y += 10;
                        printf("(%d, %d)\n", circle_centers[current_circle].x, circle_centers[current_circle].y);
                        break;
                    case 0x1b:
                        printf("ESC invoked to end calibration\n");
                        run = false;
                        break;
					case 'I':
						for (int i = 0; i < nCircles; ++i) {
							printf("CIRCLE(%d) centered at (%d, %d)\n", i, circle_centers[i].x, circle_centers[i].y);
						}
						break;
                    default:
                        printf("char = %c = %d\n", ch, ch);
