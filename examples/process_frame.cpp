...
void ProcessFrame::processImage(const cv::Mat &img_curr) {
    ...
    recoverPose(E_mat_, frame_info_prev.features_detected, feature_points_matched_curr, R_, t_, kFocalLengthPX, kPrinciplePointPX, mask_);
    ...
    if ((scale_abs_ > 0.1) &&
        std::abs(t_.at<double>(2)) > t_.at<double>(0) &&
        std::abs(t_.at<double>(2)) > t_.at<double>(1)) {
